...
if ( ! function_exists( 'jeverly_setup' ) ) :
	...
	function jeverly_setup() {
		...
		register_nav_menus(
			array(
				'menu-footer' => esc_html__( 'footer', 'jeverly' ),
			)
		);
...
function jeverly_widgets_init() {
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar-footer', 'jeverly' ),
			'id'            => 'sidebar-footer',
			'description'   => esc_html__( 'Add widgets here.', 'jeverly' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h2 class="widget-title_footer">',
			'after_title'   => '</h2>',
		)
	);
}
...
function jeverly_scripts() {
	...
	wp_enqueue_script( 'jquery.js', get_template_directory_uri() . '/build/js/allJs.js');

	wp_enqueue_style( 'jeverly-style', get_template_directory_uri() . '/build/css/style.min.css');
